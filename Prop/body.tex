...
The representations for the set type built from the classes listed in Figure~\ref{fig:type-class-set}
are relatively straightforward as they may be built from stand-alone instances of pre-existing classes.
In more complex situations, it is often necessary to construct hierarchical data structures from multiple
components.  For example, Hancock~\cite{hancock} was ...
...
\begin{mylisting}
\end{mylisting}
\caption{Synthetic data types with representation invariants.}
\label{fig:rep-synth}
%%\end{minipage}
\end{figure}
