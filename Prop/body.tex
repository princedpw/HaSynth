...
%
Library writers face particularly daunting challenges as the
...
In such a partnership targeting resource-savvy programming,
...
Loncaric \emph{et al.}~\cite{cozy} investigated synthesis of even richer data
representations with custom query methods.
Trilla and Runciman use it to improve the performance of implicit parallel
Haskell programs~\cite{implicit-parallel}, while
PI Fisher's \textsc{AutoBahn} system uses it to generate strictness
annotations to improve the run-time or memory utilization of Haskell
...
languages need new mechanisms to define abstractions suitable for
...
data structures in languages with rich type systems presents a new set of practical
...
In this case, failing to consider outliers can lead to 
unreliable performance for end
...
to the representative workload for synthesis.
...
In addition, we will advance the state of the art in (1) type theory as we
explore how best to use symbolic types to express the relevant search
spaces, and in (2) the nascent field of machine-learning enabled program
...
This proposal, if funded, would be PI Fisher's first technical NSF grant (most of her
career has been spent in industry);
...
attempts to synthesize appropriate concrete values for the symbolic
ones by using
...
implementations, but to consider such values in the context of a
language with a very expressive type system (Haskell) rather 
than an untyped language such as Racket.
...
type-directed computations.  Symbolic types may also interact with
dependent constraints and many of Haskell's advanced features such
as GADTs and type families.  A second important
...
static analyses such as bounded model checking.  Rosette also focuses on
correctness, and does not provide a means to speed up performance, whereas our
proposed system focuses on improving efficiency.  

Synapse~\cite{synapse} is a system built on top of Rosette and is designed
for optimization.  The central innovation is the search strategy, which
involves two collaborating algorithms that search through a series of 
``metasketches.''  Unlike our proposal, Synapse is not hosted by a typed
language like Haskell.  We also plan to investigate a different collection
...
not provide easy integration with a host language.  Unlike
...
in libraries or compilers~\cite[\ldots]{FrigoJo98,Im2001,Whaley+:atlas} and
gave programmers little source-level 
control over the space of programs that the compiler might work with.
Recently, \textsc{chameleon}~\cite{Shacham+:chameleon} changed this
situation by allowing
some user customization.  \textsc{chameleon} chooses an efficient collection
implementation at runtime based on program traces and the heap. Users can
customize \textsc{chameleon} by providing their own collection implementations. 
Work on PetaBricks~\cite{Ansel+:petabricks,Phothilimthana+:gpupetabricks} took a
substantial step forward by allowing \emph{source-level programmers} to specify 
...
PetaBricks has been extended to support optimizing variable-accuracy algorithms,
where one can sacrifice accuracy for efficiency, and
vice-versa~\cite{Ansel+:variablepetabricks}. 
Our proposed language design is quite
...
to new evaluation strategies for Haskell, are quite 
different as well.

%% One application area of our work is the synthesis of efficient data
%% structures, building on previous work by PI Fisher on
%% \textsc{RelC}~\cite{data-rep-synth,conc-data-rep-synth}. Given a
%% relational specification, the \textsc{RelC} system synthesizes a data
%% structure capable of holding the relational information and
%% corresponding functions for insertion, querying, and deletion.
%% \textsc{RelC} has an autotuner component that makes the synthesized
%% data structure perform well on provided examples. The Cozy
%% system~\cite{cozy} performs a similar task, synthesizing an efficient
%% data structure from a description of the data structure's fields and
%% the queries that will be performed on it.  Like \textsc{RelC}, Cozy
%% provides autotuning capabilities.  Our work differs in that the
%% previous work only allowed specific types of specifications to be
%% provided by the user.  In our proposed work users will be able to
%% build their own specification definitions, letting them easily extend
%% the synthesis system to suit new use-cases.
...
defines \cd{gen*}, a new \emph{set} of symbolic booleans.  Intuitively, every occurrence of \cd{gen*}
...
separate occurrence of \cd{gen*}.  Each occurrence will correspond to a
...
%%   set of light-weight mechanisms for the programmer to specify search
...
Other researchers have shown that for super-optimization, stochastic search is very
...
Autobahn, stochastic search, probabilistic modeling, enumerative
...
may not be able to supply optimal performance, but we may still be able to
improve application performance dramatically.
...
strategy to an application is to expose some of the parameters of the \rasp{} strategy to users.
...
The preceding paragraphs demonstrate the main elements involved in
...
triggers this analysis for the code synthesized by the Autobahn \rasp{}.
...
As we discuss below, we believe our design will be particularly lightweight and easy to integrate into
...
by symbolic types; once the symbolic types are made concrete, it should
...
%% symbolic t* :: GD
%% \end{mylisting}
%% \caption{Representation synthesis \rasp}
...
parallelism~\cite{implicit-parallel} and Fisher and collaborators' work on 
...
programming primitives while tacitly handling implementation details like establishing
...
on BlockyTalky, will design and build a \rasp system for the visual
...
These collaborations have always occurred with the PIs at separate
...
The PI has also given a recent talk at Cisco as a way of transferring academic
