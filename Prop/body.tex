...
form; if  \cd{gen*} is false, it leaves \cd{e} untouched.  Hence, any place \cd{eval?}, it
...
application (\ie, every place \cd{Apply(e1,e2)} appears in the client abstract syntax) will
possibly eagerly evaluate (and possibly lazily evaluate) its argument.  We represent the application
of this syntactic transformation via the \cd{map\_syntax} primitive.  One minor detail here is
that body of \cd{eval?} should be inlined so that every function call has its own separate occurrence
of \cd{gen*}.  Each occurence will correspond to a separate symbolic boolean, which will allow
...
Together, symbolic values, symbolic computations (such as \cd{eval?}), and optional syntax rewriting
define a broad space of possible implementations.  What is needed next are a set of \emph{strategies}
