...
end-user programming support~\cite{Harris:spread-sheets,Barowy:flashrelate} to super-optimization~\cite{stochastic-superopt} to education~\cite{Singh:education,Gulwani:cacm-education}, there has been
a great deal of recent work in the programming languages community on
new languages and algorithms for synthesizing programs.

The work we propose has been most closely inspired by 
Rosette~\cite{Torlak:rosette-pldi,Torlak:rosette-onward}, 
a ``solver-aided language,'' hosted within Racket, that allows 
users to declare and use symbolic values within Racket computations.  
Rosette attempts to synthesizes appropriate
concrete values for the symbolic ones (usually concrete values that
satisfy given assertions) by using a form of bounded model checking
and SMT solvers.  We propose to adopt the idea of using symbolic values
to specify a space of implementations, but to consider such values in
the context of a modern, typed language---Haskell---rather than an
untyped language such as Racket.  When one does so, a range of new
opportunities and challenges emerge.  In particular, we propose to
allow users to declare and use \emph{symbolic types} in addition to symbolic
values.  As we will see later in the proposal, such symbolic types
have the potential to drive generation of symbolic type classes
and type-directed computations.  They also interact with dependent
constraints and type families.  A second important difference between
our proposed work and Rosette is that we plan to explore search strategies based
on dynamic evaluation cost metrics ranging stochastic search to
reinforcement learning rather than using static analyses such as 
bounded model checking to drive synthesis.
...
\ksf{The \texttt{cd} latex command is not producing code in a distinct font. Not
  sure how to fix correctly given that \texttt{cd} macros is already defined.}
...
and polymorphic and dependent types. \dpw{It would be nice to mention Hancock somewhere.}
...
A simple example type class that defines an interface to collections is presented in
Figure~\ref{type-class-set}.  Here, we define the type class \cd{Set}, which has methods
\cd{empty} to create the empty set (the parameter gives an estimated size of the set), \cd{insert} to add a member to a set, and \cd{member}
to check for membership in a set.  We also sketch two instances (\ie, implementations) of this
class.  One implementation uses a \cd{Vector} and the other uses a red-black tree.
...
looking again at Figure~\ref{fig:type-class-set}, we see declarations of a symbolic type \cd{t*}
with kind \cd{* -> *}, indicating it is a function from types to types.  Moreover, \cd{t*} is constrained
to be in the class \cd{Set}.  Such a constraint limits the collection of possible types, making search
tractible.  Later, in \cd{component}, the type \cd{t*} is used in a typing annotation.  Doing so
implies the implementation of sets used by the \cd{process} function will be determined by resource-aware
...
\draw (p1) +(-6mm,15mm) node {\bf(a)};
\draw (p2) +(-8mm,15mm) node {\bf(b)};
...
range of possible representations \cd{R} coarsely.
...
PIs Fisher and Walker have already been collaborating successfully with one 
another for roughly a decade on projects ranging from the design and implementation
of domain-specific languages for data processing such as 
PADS~\cite{mandelbaum:pads-ml,fisher+:next-700,fisher+:pads-overview} and 
Forest~\cite{fisher+:forest} 
to the development of new algorithms 
for program 
synthesis~\cite{fisher+:learnpads,zhu+:learnpads2,miltner+:synthesizing-bijective-lenses}.  These colloborations have always occurred with the PIs at separate
institutions, as in the current proposal.
...
Due to our existing collaborations we already have a shared Github repository for code, data, and publications.  We will continue to use this repository to share and maintain our research artifacts.  The table in Figure~\ref{fig:timeline}
presents a timeline for our collaborative activities.
