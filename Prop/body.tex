...
in libraries or compilers~\cite[\ldots]{Whaley+:atlas,FrigoJo98,Im2001}, and
gave programmers little source-level 
control over the space of programs that the compiler might work with.
Recently, \textsc{chameleon}~\cite{Shacham+:chameleon} changed this by allowing
...
where one can sacrafice accuracy for efficiency, and
...
Our proposed work aims to tackle the task of synthesizing efficient data
structures.  Given a relational specification, the \textsc{RelC}
system~\cite{data-rep-synth,conc-data-rep-synth} synthesizes a data structure
capable of holding that relational information. Insertion, query, and deletion
functions that act on the synthesized data structure also provides.
\textsc{RelC} has an autotuner component that makes the synthesized data
structure perform well on provided examples. The Cozy system~\cite{cozy}
performs a similar task, synthesizing an efficient data structure from a
description of the data structure's fields and the queries that will be
performed on it.  Like \textsc{RelC}, Cozy provides autotuning capabilities.
Our work differs from previous work on synthesizing and autotuning data
structures, in that previous work only allowed specific types of specifications
to be provided by the user.  Our proposed work, however, allows users to build
their own specification definitions, letting users easily extend their synthesis
system to suit new use-cases.
