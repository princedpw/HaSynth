...
We will explore using \rasps to make is easier for educators to add new
abstractions to a visual programming language.  Visual languages, such
as Google's Blockly \cite{Blocks,BlocklyApps}, are designed to help
novice programmers write engaging software artifacts by providing
visual cues about how to assemble programs and by removing the need to
worry about syntax.  As an example of current practice, the visual
language BlockyTalky\cite{BlockyTalky} expresses distributed system
design while tacitly handling implementation details like establishing
remote connections and buffering message sends in its runtime system.
Researchers have used BlockyTalky to engage middle school students
from underrepresented backgrounds in computer science by having them
design their own electronic instruments and use them to play music.

It is useful to be able to add higher-level abstractions to visual
programming languages so that students can express more interesting
programs more quickly.  Unfortunately, adding such features is
difficult at the moment.  One way is to extend the implementation of
